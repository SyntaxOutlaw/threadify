...
 *
 * 优先级：
 *  1) /threads-order 预取的 depth (来自 ThreadOrderPrefetch)
 *  2) [REMOVED] /threads 元数据里的 _threadDepth
 *  3) 本地沿 parent_id 向上爬链计算
 *
 * 始终返回 0..MAX_DISPLAY_DEPTH，且不会抛错。
 */

// import { getPostThreadMetadata } from './ThreadsApi'; // [REMOVED] 统一数据源
...
  // [REMOVED] 移除对 getPostThreadMetadata 的依赖
  // try {
  //   const did =
  //     (post.discussion && typeof post.discussion === 'function' && post.discussion() && post.discussion().id && post.discussion().id()) ||
  //     null;
...
// [REMOVED] 以下函数依赖于已删除的 ThreadsApi

...
import { getThreadDepth } from '../utils/SimplifiedThreadDepth'; // [ADDED] 统一的逻辑核心
...
  const models  = new Map(posts.map((el) => [Number(el.dataset.id), getModel(Number(el.dataset.id))]));
  // [REMOVED] parentOf map was redundant, getThreadDepth handles all logic now

  const MAX_DEPTH = 10;
  // [REMOVED] computeDepth inlined/replaced by getThreadDepth
...
    const depth = event ? 0 : getThreadDepth(m); // [MODIFIED] 使用统一的逻辑
...
  // 2. 总是执行 CSS 检查（为了功能正确性）
  //    并记录 CSS 是否发生了变化
  const classesChanged = ensureDepthClassesForWindow(container, orderMap);

  // 3. 只有在 DOM 顺序 *或者* CSS 类名
  //    两者中 *至少有一个* 真正发生变化时，才去触发 redraw
...
      // [REMOVED] 第 3 步：移除“其它变化”兜底，解决滚动/点赞卡顿
      // 其它变化：轻量归一化
      // if (scheduled) return;
      // scheduled = true;
      // requestAnimationFrame(() => {

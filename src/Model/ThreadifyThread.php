...
        
        // [!! OPTIMIZED !!] 优化 N+1 更新：
Section 1.  收集所有祖先的 thread_path
        $ancestorPaths = [];
        while (!empty($pathParts)) {
            $ancestorPaths[] = implode('/', $pathParts);
            array_pop($pathParts);
        }
        
        // [!! OPTIMIZED !!] 优化 N+1 更新：
Section 2.  使用一个 "whereIn" 批量更新所有祖先
        if (!empty($ancestorPaths)) {
            self::where('discussion_id', $threadEntry->discussion_id)
                ->whereIn('thread_path', $ancestorPaths)
...
s          ->where(function($query) use ($thread) {
